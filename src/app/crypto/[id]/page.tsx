    ...
    <div className='w-full min-h-screen flex flex-col'>
      ...
      <div className='flex flex-1 flex-col lg:flex-row'>
        ...
        <div className='flex-1 h-full p-4'>
            ...
            <div className="grid grid-cols-4 gap-x-1 p-1 bg-slate-800 rounded-md">
              {[1, 7, 14, 30].map(item => (
                <button
                  ...
                  className={`${days === item ? "bg-slate-900" : ""} w-full px-2 py-1 rounded-md text-center text-gray-400 cursor-pointer`}
                >
                  {item}
                </button>
              ))}
          ...
          {isLoadingOHLC 
            ? <p>Loading chart</p> 
            : <Chart data={dataOHLC} chartType={chartType} profit={data?.market_data?.price_change_percentage_24h >= 0} />
          }
        </div>
      </div>
    </div>
