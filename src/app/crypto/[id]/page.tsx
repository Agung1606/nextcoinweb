...
const GridItem = ({
  ...
  return (
    <div
      className={`${className} flex flex-col items-center justify-center border border-slate-600 p-2 rounded-md`}
    >
...
function CryptoInfo({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [chartType, setChartType] = useState<"candleStick" | "chartArea">(
    "candleStick"
  );
  ...
  const { data: dataOHLC, isLoading: isLoadingOHLC } = useQuery({
    queryKey: ["coinOHLC", id, days],
    queryFn: () => fetchOHLCByID({ id: id, vs_currency: "usd", days: days }),
    refetchInterval: 30000,
    staleTime: 1000 * 60,
  });
      ...
      <div className="flex flex-1 flex-col lg:flex-row">
            ...
            <>
              ...
              <div className="flex flex-col gap-y-6">
                <div className="flex items-center gap-x-2">
                  ...
                  <span className="text-xs font-semibold bg-slate-600 px-2 rounded-md">
              ...
              <div className="grid grid-cols-2 gap-2 mt-6">
                <GridItem
                  text="Market cap"
                  data={`$${
                    millify(data?.market_data?.market_cap?.usd) ?? "N/A"
                  }`}
                ...
                <GridItem
                  text="Circ. supply"
                  data={`${
                    millify(data?.market_data?.circulating_supply) ?? "N/A"
                  }`}
                  symbol={data?.symbol.toUpperCase()}
                />
              </div>
        ...
        <div className="flex-1 h-full p-4">
          <div className="flex justify-between items-center mb-8">
            <div className="flex items-center p-1 bg-slate-800 rounded-md">
              <button
                aria-pressed={chartType === "candleStick"}
                className={`${
                  chartType === "candleStick" ? "bg-slate-900" : ""
              ...
              <button
                ...
                className={`${
                  chartType === "chartArea" ? "bg-slate-900" : ""
                } px-2 py-1 rounded-md cursor-pointer`}
            ...
            <div className="grid grid-cols-4 gap-x-1 p-1 bg-slate-800 rounded-md">
              {[1, 7, 14, 30].map((item) => (
                <button
                  key={item}
                  onClick={() => setDays(item)}
                  className={`${
                    days === item ? "bg-slate-900" : ""
                  } w-full px-2 py-1 rounded-md text-center text-gray-400 cursor-pointer`}
                >
                  {item}
                </button>
              ))}
          ...
          {isLoadingOHLC ? (
            ...
            <Chart
              data={dataOHLC}
              chartType={chartType}
              profit={data?.market_data?.price_change_percentage_24h >= 0}
            />
          )}
        </div>
      </div>
