  ...
  return (
    <Dialog.Root>
      ...
      <Dialog.Portal>
        ...
        <Dialog.Content
          className="fixed left-1/2 top-1/2 w-[50%] -translate-x-1/2 -translate-y-1/2 rounded-xl bg-[#020a18] py-2 px-4 shadow-lg focus:outline-none"
          aria-describedby="search-list"
        >
          <Dialog.Title hidden>Search Coins</Dialog.Title>
          ...
          <div className={`${isLoading && "hidden"} mt-4`}>
            ...
            {!data && trendingListData && (
              <ul className="space-y-2 max-h-96 overflow-y-auto pr-2">
                {trendingListData.coins.map((coin: any) => (
                  <CoinCard
                    key={coin.item.id}
                    id={coin.item.id}
                    ...
                    className="bg-slate-800 border border-slate-800 hover:bg-blue-900 hover:border-blue-600"
                  />
                ))}
              </ul>
            )}
              ...
              <ul className="space-y-2 max-h-96 overflow-y-auto pr-2">
                {data.map((coin: any) => (
                  <CoinCard
                    ...
                    symbol={coin.symbol}
                    className={
                      "bg-slate-800 border border-slate-800 hover:bg-blue-900 hover:border-blue-600"
                    }
                  />
                ))}
              </ul>
